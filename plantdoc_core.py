...
class PlantDocAI:
    ...
    def _get_treatment(self, disease, confidence):
        ...
        print(f"🔍 DEBUG: _get_treatment called with disease='{disease}', confidence={confidence}")
        ...
        print(f"🔍 DEBUG: Confidence level determined as '{level}'")
        print(f"🔍 DEBUG: Available diseases in treatments: {list(treatments.keys())}")
        
        # Get appropriate treatment or fallback
        disease_treatments = treatments.get(disease, {})
        print(f"🔍 DEBUG: Found treatments for '{disease}': {bool(disease_treatments)}")
        
        if disease_treatments:
            print(f"🔍 DEBUG: Available levels for '{disease}': {list(disease_treatments.keys())}")
        
        treatment = disease_treatments.get(level, f'Consult agricultural specialist for {disease} treatment (confidence: {confidence:.1%}).')
        print(f"🔍 DEBUG: Final treatment: '{treatment[:50]}...'")
